...
void afficher_historique_emprunts(const Emprunt *emprunts, int nb_emprunts) {
    ...
    for (int i = 0; i < nb_emprunts; i++) {
        printf("  | %-4d | %-7d | %-11d | %-16s | %-16s | %-8s | %-6d |\n",
               emprunts[i].id, emprunts[i].id_livre, emprunts[i].id_emprunteur,
               emprunts[i].date_emprunt, emprunts[i].date_retour_prevue,
               emprunts[i].est_retourne ? "OUI" : "NON", emprunts[i].amendes);
    }
    printf("  +------+---------+-------------+------------------+------------------+----------+--------+\n");
